...
class GWords100(db.Model):
  lines = db.TextProperty(default="")
 
class GWords1K(db.Model):
  lines = db.TextProperty(default="")

class GWords10K(db.Model):
  lines = db.TextProperty(default="")

class GWords100K(db.Model):
  lines = db.TextProperty(default="")

class GWords1M(db.Model):
  lines = db.TextProperty(default="")

class GWords10M(db.Model):
  lines = db.TextProperty(default="")

""" These entries are a part of a run of Grep """
class GrepResults(db.Model):
  value = db.StringProperty(indexed=False)
  needle = db.StringProperty(indexed=False)
...
def get_result_query(num_entries):
  if num_entries >= 1000000:
    return GWordCount1M.all()
  elif num_entries >= 100000:
    return GWordCount100K.all()
  elif num_entries >= 10000:
    return GWordCount10K.all()
  elif num_entries >= 1000:
    return GWordCount1K.all()
  elif num_entries >= 100:
    return GWordCount100.all()

""" Information of a dataset, such as how long it took to create """
class GrepDataSet(db.Model):
...
def gen_data(num_entries, user, name, char_per_word, entries_per_pipe):
  ...
  dataset = GrepDataSet(name=name, num_entries=num_entries,
...
class GenerateData(webapp.RequestHandler):
  def get(self):
    ...
    self.response.out.write(template.render("templates/grep_data.html",
                                            {"user": user.email(),
  ...
  def post(self):
    ...
    if self.request.get("generate"):
      ...
      self.redirect('/data/grep')
      # pipeline seems broken
      #self.redirect(route)
    elif self.request.get("delete"):
      name = self.request.get("name") 
      dataset = GrepDataSet.get_by_key_name(name)
...
def getNewEntry(num_entries, keyname, value):
  if num_entries >= 1000000:
    return GWords1M(key_name=keyname, lines=value)
  elif num_entries >= 100000:
    return GWords100K(key_name=keyname, lines=value)
  elif num_entries >= 10000:
    return GWords10K(key_name=keyname, lines=value)
  elif num_entries >= 1000:
    return GWords1K(key_name=keyname, lines=value)
  else:
    return GWords100(key_name=keyname, lines=value)
...
class GenDone(pipeline.Pipeline):
  ...
  def run(self, name, *results):
    ...
    ds = GrepDataSet.get_by_key_name(name)
    then = ds.start
